...
for pid in pids:
    ...
    cv_scores_pid = ml_utils.csp_lda_cv_per_participant(
        X_epoched=X_epoched, y=y, fs=fs, csp_params=csp_params
    )
    ...
    csp_filter_weights, lda = ml_utils.csp_lda_final_model(
        X_epoched=X_epoched, y=y, fs=fs, csp_params=csp_params
    )
    ...
    joblib.dump(
        {
            "cross_val_scores": cv_scores_pid,
            "csp_weights": csp_filter_weights,
            "lda": lda,
        },
        f"{result_dir}/csp_lda_model_{pid}.joblib",
    )
